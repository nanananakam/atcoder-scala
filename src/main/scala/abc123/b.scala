...
object Main extends App {
  val a = Array.fill(5)(Scanner.nextInt)
  val fractionMinIndex = a.zipWithIndex.minBy(ai=>{
    if (ai._1%10 == 0){
      10
    } else {
      ai._1%10
  ...
  val result = a.zipWithIndex.map(ai=>{
    if (ai._2 != fractionMinIndex) {
      (scala.math.ceil(ai._1/10.0)*10).toInt
    } else {
      ai._1
    }
  }).sum
